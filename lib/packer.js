/**
 * got this from https://github.com/Siorki/RegPack
 */
...
module.exports = callRegPack = function(input, globalVar, variables, withMath) {
	...
	var options = {
		...
		crushTiebreakerFactor : parseInt(1) // || -1
	};
	var varsNotReassignedRaw = variables;
	for (var i=0; i<127; ++i) {
...
RegPack.prototype = {
	...
	preprocess : function(input, options) {
		...
		if (options.hash2DContext) {
			...
			{
				var result = this.preprocess2DContext(inputList[i][1][1], (options.contextType==0?options.contextVariableName:false), context);
				if (result) {
		...
		if (options.hashWebGLContext) {
			...
			for (var count=inputList.length, i=0; i<count; ++i)
			{
				var result = this.preprocessWebGLContext(inputList[i][1][1], (options.contextType==1?options.contextVariableName:false), context);
				if (result) {
					result[2]=inputList[i][1][2]+result[2];
		...
		if (options.hashAudioContext) {
			...
			for (var count=inputList.length, i=0; i<count; ++i) 
			{
				var result = this.preprocessAudioContext(inputList[i][1][1], context);
				if (result) {
					result[2]=inputList[i][1][2]+result[2];
	 ...
	 */
	preprocess2DContext : function(input, variableName, context) {
		...
		if (contextFound) {
			var preprocessedCode = this.renameObjectMethods(input, objectName, objectOffset, declarationLength, context);
	 ...
	 */
	preprocessWebGLContext : function(input, variableName, context) {
		// Obtain the context definition (variable name and location in the code)
		var objectName = variableName, objectOffset = 0, declarationLength = 0, contextFound = false;
		if (variableName) {
			// either it is provided to us
	...
	preprocessAudioContext : function(input, context) {
		...
		if (replacementOffset>0) {
			var preprocessedCode = this.renameObjectMethods(input, objectName, replacementOffset, 0, context);
	 ...
	 */
	renameObjectMethods : function(input, objectName, offset, declarationLength, reference)
	{
		...
		for (var functionIndex=0; functionIndex<hashFunctions.length; ++functionIndex) {
			for (var yValue=1; yValue<=hashFunctions[functionIndex][1] ; ++yValue) {
				...
				for (var index=0; index<methodsInUse.length; ++index) {
					if (forwardLookup[methodsInUse[index]]) {
						score += methodsInUse[index].length - forwardLookup[methodsInUse[index]].length;
					}
				}
				score-=hashFunctions[functionIndex][0].length;
		...
		for (var index=0; index<methodsInUse.length; ++index) {
			if (methodsInUse[index] in forwardLookup) {
...
function test() {
	var stri = '(function(_,o,n,T,S,a,q,x,k,p,b,e,w){ for(i=0,P=r();i<24;i++){ while(~w[_](P)||P==q||!P)P=r(); w[i]=P } function r(){ return~~(Math.random()*(e+1)) } function d(){ s="<pre style=border:solid;width:350px>";q<0&&(q=e);p<0&&(p=e);p>e&&(p=e-b); for(i=0;i<e;i++){ if(i^0&&i%b==0)s+="";R=0; for(y in w)if(w[y]==i)s+="~|~",R++; s+=i==p?" > ":q==i?" o ":!R?" ":"" } document.body.innerHTML=s+"\n P: "+x+"\t\t\t\t O2: "+~~((1-a/T)*100)+"%" } onkeyup=function(e){ h=e.which; g=h==39?p+1:h==37?p-1:h==40?p+b:h==38?p-b:0; if(!~w[_](g))p=g; if(q==g)x+=5,a-=S,m(),n(k),k=o(function(){m();d()},3e3); d() }; function m(){ q=w[0]; while(~w[_](q))q=r();d() } o(function(){ a+=S; if(a>=T)n(k),onkeyup=0,a=T,d(); else { for(i in w)w[i]=w[i]-1<0?e:w[i]-1,w[i]==q&&(q--),w[i]==p&&(p--); d(); } },S) })("indexOf",setInterval,clearInterval,6e4,600,0,5,0,0,0,15,255,[])';
	callRegPack(stri, 'c', 'a');
}